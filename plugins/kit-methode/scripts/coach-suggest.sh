...
# Reads <project>/.claude/logs/activity.log (populated by activity-log.sh).
# Plugin form: operates on the consuming project via ${CLAUDE_PROJECT_DIR}.
#
# NOTE: once installed as a plugin, the real command names are namespaced
# (e.g. /kit-methode:ship, /kit-methode:audit-quick). The bare /... strings
# emitted below are kept for readability; see the plugin README.
#
